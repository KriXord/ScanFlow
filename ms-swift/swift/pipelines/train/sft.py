...
class SwiftSft(SwiftPipeline, TunerMixin):
    ...
    def _prepare_model_tokenizer(self, **kwargs):
        ...
        logger.info(
            f"model_info basic: "
            f"model_type={getattr(model_info, 'model_type', None)}, "
            f"torch_dtype={getattr(model_info, 'torch_dtype', None)}, "
            f"max_model_len={getattr(model_info, 'max_model_len', None)}"
        )
        
        def _find_torch_dtype(obj, path="root", visited=None):
            if visited is None:
                visited = set()
            ...
            if isinstance(obj, torch.dtype):
                logger.warning(f"[dtype-debug] {path} = {obj}")
                return
        
            if isinstance(obj, dict):
                for key, value in obj.items():
                    _find_torch_dtype(value, f"{path}.{key}", visited)
        
            elif isinstance(obj, (list, tuple)):
                for i, value in enumerate(obj):
                    _find_torch_dtype(value, f"{path}[{i}]", visited)
        ...
        _find_torch_dtype(model_info, "model_info")
