...
# def _scanflow_debug_param(name, p):
#     import torch
#     with torch.no_grad():
#         t = p.detach()
...
#             tf = t[finite].float()
#             msg += f" min={tf.min().item():.6g} max={tf.max().item():.6g} mean={tf.mean().item():.6g}"
#         print(msg, flush=True)


# def _scanflow_reset_new_params_if_requested(model):
#     import os
#     import torch

#     if os.getenv("SCANFLOW_RESET_NEW_PARAMS", "0") != "1":
#         return

#     base_model = model.module if hasattr(model, "module") else model

#     qwen = None
#     if hasattr(base_model, "model") and hasattr(base_model.model, "qwen2_model"):
#         qwen = base_model.model.qwen2_model
#     elif hasattr(base_model, "qwen2_model"):
#         qwen = base_model.qwen2_model

#     if qwen is None:
#         print("[scanflow load check] qwen2_model not found; skip reset", flush=True)
#         return

#     print("[scanflow load check] before reset:", flush=True)
#     if hasattr(qwen, "scanpath_start"):
#         _scanflow_debug_param("qwen2_model.scanpath_start", qwen.scanpath_start)
#     if hasattr(qwen, "text_query_residual_scale"):
#         _scanflow_debug_param("qwen2_model.text_query_residual_scale", qwen.text_query_residual_scale)

#     if hasattr(qwen, "reset_scanflow_parameters"):
#         qwen.reset_scanflow_parameters()
#         print("[scanflow load check] called qwen.reset_scanflow_parameters()", flush=True)
#     else:
#         print("[scanflow load check] reset_scanflow_parameters missing; doing minimal reset", flush=True)
#         with torch.no_grad():
#             if hasattr(qwen, "scanpath_start"):
#                 qwen.scanpath_start.zero_()
#             if hasattr(qwen, "text_query_residual_scale"):
#                 qwen.text_query_residual_scale.fill_(1e-3)
#             if hasattr(qwen, "fixation_decoder") and hasattr(qwen.fixation_decoder, "logit_scale"):
#                 qwen.fixation_decoder.logit_scale.fill_(1.0)

#     print("[scanflow load check] after reset:", flush=True)
#     if hasattr(qwen, "scanpath_start"):
#         _scanflow_debug_param("qwen2_model.scanpath_start", qwen.scanpath_start)
#     if hasattr(qwen, "text_query_residual_scale"):
#         _scanflow_debug_param("qwen2_model.text_query_residual_scale", qwen.text_query_residual_scale)
...
def _scanflow_debug_param(name, p):
    ...
    if p is None:
        print(
            f"[scanflow load check] {name}: None; skipped",
            flush=True,
        )
        return
    ...
    if not torch.is_tensor(p):
        print(
            f"[scanflow load check] {name}: "
            f"unsupported type={type(p).__name__}; skipped",
            flush=True,
        )
        return
...
def _scanflow_reset_new_params_if_requested(model):
    ...
    if (
        ...
        and hasattr(base_model.model, "qwen2_model")
    ):
        qwen = base_model.model.qwen2_model
    elif hasattr(base_model, "qwen2_model"):
        qwen = base_model.qwen2_model
    ...
    if qwen is None:
        print(
            "[scanflow load check] qwen2_model not found; skip reset",
            flush=True,
        )
        return

    def debug_current_parameters(stage):
        print(f"[scanflow load check] {stage} reset:", flush=True)

        # Older attention/fixation architecture. This attribute may exist but
        # intentionally be None when global-pooled initialization is used.
        _scanflow_debug_param(
            "qwen2_model.scanpath_start",
            getattr(qwen, "scanpath_start", None),
        )

        _scanflow_debug_param(
            "qwen2_model.text_query_residual_scale",
            getattr(qwen, "text_query_residual_scale", None),
        )

        # Old trainable fixation decoder.
        fixation_decoder = getattr(qwen, "fixation_decoder", None)
        _scanflow_debug_param(
            "qwen2_model.fixation_decoder.logit_scale",
            getattr(fixation_decoder, "logit_scale", None),
        )

        # New routing architecture.
        evidence_router = getattr(qwen, "evidence_router", None)
        query_proj = getattr(evidence_router, "query_proj", None)
        _scanflow_debug_param(
            "qwen2_model.evidence_router.query_proj.weight",
            getattr(query_proj, "weight", None),
        )

        scanpath_blocks = getattr(qwen, "scanpath_blocks", None)
        if scanpath_blocks is not None and len(scanpath_blocks) > 0:
            evidence_proj = getattr(
                scanpath_blocks[0],
                "evidence_proj",
                None,
            )
            _scanflow_debug_param(
                "qwen2_model.scanpath_blocks.0.evidence_proj.weight",
                getattr(evidence_proj, "weight", None),
            )

    debug_current_parameters("before")

    if hasattr(qwen, "reset_scanflow_parameters"):
        qwen.reset_scanflow_parameters()
        print(
            "[scanflow load check] "
            "called qwen.reset_scanflow_parameters()",
            flush=True,
        )
    else:
        print(
            "[scanflow load check] reset_scanflow_parameters missing; "
            "performing minimal compatible reset",
            flush=True,
        )

        with torch.no_grad():
            scanpath_start = getattr(qwen, "scanpath_start", None)
            if scanpath_start is not None:
                scanpath_start.zero_()

            residual_scale = getattr(
                qwen,
                "text_query_residual_scale",
                None,
            )
            if residual_scale is not None:
                residual_scale.fill_(1e-3)

            fixation_decoder = getattr(qwen, "fixation_decoder", None)
            fixation_logit_scale = getattr(
                fixation_decoder,
                "logit_scale",
                None,
            )
            if fixation_logit_scale is not None:
                fixation_logit_scale.fill_(1.0)

            evidence_router = getattr(qwen, "evidence_router", None)
            if (
                evidence_router is not None
                and hasattr(evidence_router, "reset_parameters")
            ):
                evidence_router.reset_parameters()

    debug_current_parameters("after")
    ...
    def get_model(self, model_dir: str, config: PretrainedConfig, processor: Processor,
        ...
        if model is None:
            ...
            with context():
                ...
                _scanflow_reset_new_params_if_requested(model)
