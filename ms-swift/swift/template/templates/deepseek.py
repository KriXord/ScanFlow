...
class DeepseekOCR(Template):
    ...
    def init_env_args(self):
        ...
        # ScanFlow dynamic causal-token template mode.
        #
        # Default=False preserves the ORIGINAL DeepSeek-OCR2 fixed image-token
        # layout (144 local / 256 global causal features), which is required by
        # the original model and Parallel Plan 1.
        #
        # Dynamic mode is required whenever the visual causal encoder emits a
        # sample-specific recurrent trajectory of length T instead of the fixed
        # 144/256 causal-query sequence. This includes:
        #   - ScanFlow Plan 2
        #   - ScanFlow Recurrent Plan 1
        #
        # Preferred switch:
        #   export SCANFLOW_DYNAMIC_CAUSAL_TOKENS=1
        #
        # Backward compatibility:
        #   export SCANFLOW_PLAN2=1
        # still enables the same dynamic-token behavior for older Plan-2 jobs.
        legacy_scanflow_plan2 = get_env_args(
            'scanflow_plan2',
            bool,
            False,
        )
        self.scanflow_dynamic_causal_tokens = get_env_args(
            'scanflow_dynamic_causal_tokens',
            bool,
            legacy_scanflow_plan2,
        )

        # ScanFlow Pro V4 returns [M^T; H] for every encoded view rather than H
        # alone. Keep this as a separate opt-in so the existing dynamic-T layout
        # remains unchanged for Plan 2 and recurrent Plan 1.
        #
        #   export SCANFLOW_PRO_V4=1
        #
        # V4 necessarily uses dynamic scanpath length T, so enabling it also
        # enables the common dynamic-causal-token preprocessing path.
        self.scanflow_pro_v4 = get_env_args(
            'scanflow_pro_v4',
            bool,
            False,
        )
        if self.scanflow_pro_v4:
            self.scanflow_dynamic_causal_tokens = True

        # Keep the old attribute as an alias for any external code that still
        # reads it, but all template logic below uses the architecture-neutral
        # dynamic-causal-token flag.
        self.scanflow_plan2 = self.scanflow_dynamic_causal_tokens
    ...
    def _preprocess_image(
        ...
        for image in images:
            if crop_mode:
                ...
                """add image tokens"""
                if self.scanflow_dynamic_causal_tokens:
                    if self.version != 'v2':
                        raise ValueError(
                            "scanflow_dynamic_causal_tokens is only supported for the DeepSeek-OCR2 template."
                        )
                    if scanpath_steps is None:
                        raise ValueError(
                            "scanflow_dynamic_causal_tokens=1 requires scanpath_steps for image preprocessing."
                        )
                    ...
                    if self.scanflow_pro_v4:
                        # qwen2_model returns [M^T; H] independently for the
                        # global view and every local crop.
                        global_view_tokens = (
                            num_queries_base ** 2 + int(scanpath_steps)
                        )
                        local_view_tokens = (
                            num_queries ** 2 + int(scanpath_steps)
                        )
                        num_dynamic_tokens = (
                            global_view_tokens
                            + num_local_views * local_view_tokens
                            + 1
                        )  # +1 existing view separator
                    else:
                        # Existing Plan-2 / recurrent-Plan-1 layout: H only.
                        num_dynamic_tokens = (
                            int(scanpath_steps) * (1 + num_local_views) + 1
                        )
                    tokenized_image = [image_token_id] * num_dynamic_tokens
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        ...
        # Dynamic ScanFlow architectures normally emit T causal features per
        # encoded view. ScanFlow Pro V4 instead emits [M^T; H], requiring N+T
        # slots per view. The template reserves the appropriate layout for that
        # sample BEFORE batching. T is read from the same
        # extra_kwargs.scanpath_length that the base collator later converts
        # into model.forward(scanpath_lengths=...).
        #
        # Fixed-token architectures (original DeepSeek-OCR2 and Parallel
        # Plan 1) leave this disabled and retain the original 144/256 layout.
        scanpath_steps = None
        if self.scanflow_dynamic_causal_tokens:
            if self.version != 'v2':
                raise ValueError(
            ...
            extra_kwargs = getattr(inputs, 'extra_kwargs', None) or {}
            raw_scanpath_length = extra_kwargs.get('scanpath_length')

            if raw_scanpath_length is not None:
                if torch.is_tensor(raw_scanpath_length):
                    if raw_scanpath_length.numel() != 1:
                        raise ValueError(
                            "Expected one scanpath_length per encoded sample, "
                            f"got tensor shape={tuple(raw_scanpath_length.shape)}."
                        )
                    raw_scanpath_length = raw_scanpath_length.item()
                elif isinstance(raw_scanpath_length, (list, tuple)):
                    if len(raw_scanpath_length) != 1:
                        raise ValueError(
                            "Expected one scanpath_length per encoded sample, "
                            f"got {len(raw_scanpath_length)} values."
                        )
                    raw_scanpath_length = raw_scanpath_length[0]

                scanpath_steps = int(raw_scanpath_length)
                if scanpath_steps < 1 or scanpath_steps > 32:
                    raise ValueError(
                        f"scanpath_length={scanpath_steps} is outside the "
                        "supported dynamic ScanFlow range [1, 32]."
                    )
            else:
                scanpath_steps = 32
