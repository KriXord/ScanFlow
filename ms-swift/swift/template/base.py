...
class Template(ProcessorMixin):
    ...
    def encode(self,
        ...
        if chosen.extra_kwargs:
            print("[scanflow extra_kwargs]", chosen.extra_kwargs.keys(), flush=True)
    ...
    def data_collator(self, batch: List[Dict[str, Any]], *, padding_to: Optional[int] = None) -> Dict[str, Any]:
        ...
        if not self.remove_unused_columns:
            ...
            extra_kwargs = (
                RowPreprocessor.rows_to_batched(extra_kwargs_list)
                if extra_kwargs_list
                else {}
            )

            fixation_map_paths = extra_kwargs.get("fixation_map_path")

            if fixation_map_paths is not None:
                import numpy as np
                import torch

                if isinstance(fixation_map_paths, str):
                    fixation_map_paths = [fixation_map_paths]

                raw_scanpath_lengths = extra_kwargs.get("scanpath_length")
                if raw_scanpath_lengths is not None and not isinstance(
                    raw_scanpath_lengths, (list, tuple)
                ):
                    raw_scanpath_lengths = [raw_scanpath_lengths]

                fixation_targets = []
                fixation_masks = []
                scanpath_lengths = []

                for i, npz_path in enumerate(fixation_map_paths):
                    data = np.load(npz_path, allow_pickle=True)

                    fixation_targets.append(
                        torch.from_numpy(data["fixation_targets"]).float()
                    )
                    fixation_masks.append(
                        torch.from_numpy(data["fixation_mask"]).bool()
                    )

                    if raw_scanpath_lengths is not None:
                        scanpath_lengths.append(int(raw_scanpath_lengths[i]))
                    else:
                        scanpath_lengths.append(int(data["scanpath_length"]))

                res["fixation_targets"] = torch.stack(fixation_targets, dim=0)
                res["fixation_mask"] = torch.stack(fixation_masks, dim=0)
                res["scanpath_lengths"] = torch.tensor(scanpath_lengths, dtype=torch.long)

                if not getattr(self, "_scanflow_printed_batch", False):
                    print("[scanflow collator] fixation_targets", res["fixation_targets"].shape, flush=True)
                    print("[scanflow collator] fixation_mask", res["fixation_mask"].shape, flush=True)
                    print("[scanflow collator] scanpath_lengths", res["scanpath_lengths"].shape, flush=True)
                    self._scanflow_printed_batch = True
            else:
                # Keep dynamic ScanFlow token reservation and model rollout in
                # sync even for datasets without fixation-map supervision. The
                # template reads singular per-row ``scanpath_length`` while the
                # model forward expects batched ``scanpath_lengths``.
                raw_scanpath_lengths = extra_kwargs.get("scanpath_length")
                if raw_scanpath_lengths is not None:
                    if torch.is_tensor(raw_scanpath_lengths):
                        scanpath_lengths = raw_scanpath_lengths.to(
                            dtype=torch.long
                        ).reshape(-1)
                    else:
                        if not isinstance(raw_scanpath_lengths, (list, tuple)):
                            raw_scanpath_lengths = [raw_scanpath_lengths]
                        scanpath_lengths = torch.tensor(
                            [int(value) for value in raw_scanpath_lengths],
                            dtype=torch.long,
                        )
                    if scanpath_lengths.numel() != num_samples:
                        raise ValueError(
                            "Expected one scanpath_length per collated sample, "
                            f"got {scanpath_lengths.numel()} lengths for "
                            f"{num_samples} samples."
                        )
                    res["scanpath_lengths"] = scanpath_lengths

                res.update({
                    k: v
                    for k, v in extra_kwargs.items()
                    if k not in res and k != "scanpath_length"
                })
