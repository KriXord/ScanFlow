...
class Qwen2Decoder2Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.recurrent_step_embed_768 = nn.Embedding(144, hidden_dimension)
        self.recurrent_step_embed_1024 = nn.Embedding(256, hidden_dimension)

        self.text_query_rnn = nn.GRU(
        ...
        self.text_query_out = nn.Linear(hidden_dimension, hidden_dimension * 2)
    ...
    def _apply_text_context(
        self,
        base_query_imgs: torch.Tensor,
        text_context: Optional[torch.Tensor],
    ) -> torch.Tensor:
        if not self.enable_text_query_residual or text_context is None:
            return base_query_imgs

        bs, n_query, hidden_dim = base_query_imgs.shape

        if n_query == 144:
            step_embed = self.recurrent_step_embed_768.weight
        elif n_query == 256:
            step_embed = self.recurrent_step_embed_1024.weight
        else:
            raise ValueError(f"Unsupported n_query={n_query}. Expected 144 or 256.")

        dtype = base_query_imgs.dtype
        device = base_query_imgs.device
        ...
        step_embed = step_embed.to(device=device, dtype=dtype)
        step_embed = step_embed.unsqueeze(0).expand(bs, -1, -1)  # (B, M, D)

        rnn_input = step_embed + text_context.unsqueeze(1)        # (B, M, D)

        recurrent_hidden, _ = self.text_query_rnn(rnn_input)      # (B, M, D)

        delta, gate = self.text_query_out(recurrent_hidden).chunk(2, dim=-1)

        delta = delta.to(dtype=dtype)
        gate = gate.to(dtype=dtype)

        residual = torch.sigmoid(gate) * delta                    # (B, M, D)

        return base_query_imgs + self.text_query_residual_scale.to(dtype=dtype) * residual
